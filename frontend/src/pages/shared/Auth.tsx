...
const Auth: React.FC<AuthProps> = () => {
  ...
  const handleMetaMaskConnect = async () => {
    ...
    try {
      ...
      setError(
        "Failed to connect to MetaMask. Please try again. Error: " + err,
      );
      setShouldShake(true);
      setTimeout(() => setShouldShake(false), 500);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-neutral-50 flex items-center justify-center p-4 overflow-x-hidden">
      <div
        className={`relative w-full max-w-2xl mx-auto transition-all duration-500 ease-in-out ${shouldShake ? "animate-shake" : ""}`}
      >
        <div className="flex w-full items-stretch">
          {/* Login Form */}
          <div
            className={`transition-all duration-500 ease-in-out ${showWallet ? "w-1/2 pr-2" : "w-full"}`}
          >
            <div
              className={`card h-full ${showWallet ? "p-4" : "p-6"} shadow-md`}
            >
              <h3
                className={`${showWallet ? "text-lg" : "text-xl"} font-semibold mb-4 text-center md:text-left`}
              >
                Enter Login Credentials
              </h3>
              <p
                className={`text-neutral-600 mb-6 text-center md:text-left ${showWallet ? "text-sm" : ""}`}
              >
          ...
          {/* Wallet Connection */}
          <div
            className={`transition-all duration-500 ease-in-out ${showWallet ? "w-1/2 pl-2 opacity-100" : "w-0 pl-0 opacity-0"}`}
          >
            <div
              className="card h-full p-4 shadow-md"
              style={{ minHeight: "100%" }}
            >
