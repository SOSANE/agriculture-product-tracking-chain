  ...
  async registerProduct(productData) {
    ...
    const query = `
            WITH profile_info AS (SELECT p.username,
                 ...
                 product_insert AS (
            INSERT
            INTO supplychain.product(id, name, description, type, image_url, batch_id, qr_code, qr_image,
            ...
            SELECT $1,
                   ...
                   0,
                   NULL
            FROM profile_info pi RETURNING *
        ),
        supplychain_insert AS (
            INSERT
            INTO supplychain.supply_chain(id, product_id, timestamp, action, description,
                                          performed_by_id, performed_by_name,
                                          performed_by_role, performed_by_organization,
                                          location_id, temperature, humidity, verified)
            SELECT
                'step1', $1, CURRENT_TIMESTAMP,
                         $10::product_status,
                         $3, pi.username, pi.name, 'farmer', pi.organization, pi.location_id, NULLIF (
                         $11, ''):: DECIMAL (5, 2), NULLIF (
                         $12, ''):: DECIMAL (5, 2), TRUE
